...
const Sidebar = (props) => {
    ...
    const userRole = useSelector(content => {
        return content.auth.loginUserData ? content.auth.loginUserData.role : null;
    });

    console.log("!!!!!!!!!!props", props);

    const adminName = useSelector(content => content.auth.loginUserData ? content.auth.loginUserData.first_name ? content.auth.loginUserData.first_name : content.auth.loginUserData.username : "ADMIN" );
        ...
        <Fragment>
            {
                window.innerWidth < 992 ? (
                    ...
                    <div className="sidebar open custom-scrollbar">
                        ...
                        >
                                ...
                                onClick={(wrapper === 'horizontal_sidebar' && layout === 'rtl') ? scrollToLeftRTL : scrollToLeft}><i className="fa fa-angle-left"></i></li>
                            {
                                MENUITEMS.map((menuItem, i) => {
                                    if((menuItem.isAdmin) && userRole !== "ADMIN") {
                                        return null;
                                    }else if ((!(menuItem.isShow) && (userRole !== "ADMIN" && userRole !== "admin"))) {
                                        return null;
                                    }
                                    else{
                                        return (
